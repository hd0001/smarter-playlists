...
def create_normalised_tables(db, schema_name):
    ...
    db.execute("CREATE TABLE IF NOT EXISTS {0}.ownership ("
               "ownership_id BIGINT GENERATED BY DEFAULT AS IDENTITY, "
               "album_id BIGINT NOT NULL, "
               "owned BOOLEAN NOT NULL, "
               "CONSTRAINT pk_ownership PRIMARY KEY (ownership_id), "
               "CONSTRAINT fk_ownership_album_id FOREIGN KEY (album_id) REFERENCES {0}.album (album_id), "
               "CONSTRAINT ck_ownership_true CHECK (owned IS TRUE), "
               "CONSTRAINT uk_ownership_album_id UNIQUE (album_id));"
               .format(schema_name))
    
    db.execute("DROP VIEW IF EXISTS {0}.ranked_album_list;".format(schema_name))
    db.execute("CREATE VIEW {0}.ranked_album_list AS "
               "SELECT "
	               "row_number() OVER (ORDER BY sum(play_count) DESC) AS rank, "
                   "al.album_id, "
	               "al.album_name, "
	               "a.artist_name, "
	               ...
	               "COALESCE(ow.owned, FALSE) AS owned "
               "FROM "
               	   "track t "
               	   "JOIN {0}.artist a ON t.artist_id = a.artist_id "
               	   "JOIN {0}.album al ON al.album_id = t.album_id "
               	   "LEFT JOIN {0}.ownership ow ON al.album_id = ow.album_id "
               "WHERE (last_played IS NOT NULL OR NOT playlist_only) "
               "GROUP BY "
               	   "al.album_name, "
                   "al.album_id, "
                   "a.artist_name, "
...
def normalise_data(db, schema_name):
    ...
    db.execute("UPDATE {0}.track t "
               ...
               "       apple_music = i.apple_music, "
               "       genre = i.genre, "
               "       playlist_only = i.playlist_only, "
               ...
               "               persistent_id, "
               "               COALESCE(apple_music :: BOOLEAN, FALSE) AS apple_music, "
               "               genre, "
               "               COALESCE(playlist_only :: BOOLEAN, FALSE) AS playlist_only, "
               "               COALESCE(skip_count :: INT, 0) AS skip_count "
